/**
 * Streamable HTTP transport runner — the primary transport for the hosted
 * Unraid "App". Runs a stateless JSON HTTP endpoint at `POST /mcp` plus a
 * `GET /healthz` liveness probe for container orchestration.
 *
 * Security: validates the `Origin` header (DNS-rebinding protection) and
 * optionally enforces a bearer token, since this transport is network-exposed.
 *
 * @see https://modelcontextprotocol.io/specification/draft.md
 */
...
export async function startHttp(
  ...
  const closeServer = await new Promise<() => Promise<void>>((resolve) => {
...
async function handleMcpRequest(
  ...
  // Stateless mode: omit sessionIdGenerator entirely (a fresh server/transport
  // per request). enableJsonResponse returns plain JSON rather than an SSE stream.
